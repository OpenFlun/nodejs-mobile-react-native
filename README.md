...
```bash
npm install @flun/nodejs-mobile-react-native
```
...
通常你不需要手动安装本包——它是 [`@flun/node-mobile-app`](https://www.npmjs.com/package/@flun/node-mobile-app) 的依赖，由 CLI 在构建时自动调用。
...
│   ├── libnode/include/node/     # 官方 v18 头文件（替换 libnode 时整目录替换）
│   └── NodeMobile.xcframework/   # iOS 用的 Node 静态库（多架构）
│
...
│   ├── create-node-structure.js     # postinstall：把 install/resources/nodejs-assets 复制到宿主项目根
...
├── publish.ps1                   # 发布辅助脚本（仅本地开发，不随包发布）
├── .vscode/                      # VSCode 工作区配置（仅本地开发，不随包发布）
└── .vs/                          # Visual Studio 工作区配置（仅本地开发，不随包发布）
```
